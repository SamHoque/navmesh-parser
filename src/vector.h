...
public:
   ...
   */
  bool
  operator==(const Arr_t &other) const
  {
    for (size_t i = 0; i < T_Size; ++i) {
      if (at(i) != other.at(i)) {
        return false;
   ...
   */
  T
  get_distance(const Vec_t &other) const
  {
   ...
   */
  Vec_t
  cross_product(const Vec_t &other,
                const bool   normalize_cross_product = false) const
  {
    static_assert(
        T_Size == 3,
        "The cross product can only be calculated for 3 dimensional vectors");
    auto vec = Vec_t(at(1) * other(2) - at(2) * other(1),
  ...
  T
  dot_product(const Vector<T2, T2_Size> &other) const
  {
...
using Vector3_t = Vector<float, 3>;
using Vector2_t = Vector<float, 2>;